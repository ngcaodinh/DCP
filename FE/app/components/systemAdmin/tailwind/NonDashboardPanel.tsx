...
function normalizeProjectReviewItem(
  ...
  return {
    ...
    description:
      typeof p.description === "string" ? p.description : "Chưa có mô tả.",
    ...
    evidenceCids: Array.isArray(p.evidenceCids)
      ? p.evidenceCids.filter((c): c is string => typeof c === "string")
      : [],
  };
}
...
function ProjectReviewPanel() {
  ...
  const selectedProject =
    projectReviewList.find((p) => p.projectId === selectedProjectId) || null;
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-xl border border-emerald-900/15 bg-white px-6 py-4">
        ...
        <p className="mt-0.5 text-xs text-slate-500">
          Danh sách dự án chờ phê duyệt — dữ liệu từ backend
        </p>
      </div>
      ...
      <div className="grid gap-4 overflow-hidden rounded-xl border border-emerald-900/15 bg-white p-4 lg:grid-cols-[300px_1fr]">
        ...
        <div className="max-h-[560px] overflow-y-auto border-r border-slate-100 pr-3">
          {isLoading ? (
            <p className="text-xs text-slate-500">Đang tải dự án...</p>
          ) : null}

          {!isLoading && projectReviewList.length === 0 ? (
            <p className="text-xs text-slate-500">Không có dự án chờ duyệt.</p>
          ) : null}
            ...
            <div className="space-y-4">
              ...
              {selectedProject.evidenceCids.length > 0 && (
                <div className="rounded-lg border border-slate-200 p-3">
                  ...
                  <div className="mt-2 grid gap-4 grid-cols-1 xl:grid-cols-2">{selectedProject.evidenceCids.map(
                        (cid, idx) => (
                          <IpfsEvidencePreviewCard
      ...
      {isApproveConfirmModalVisible && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Bạn có chắc chắn muốn phê duyệt dự án này?
            </p>
