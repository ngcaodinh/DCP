  ...
  const loadPendingSubmissionList = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 401) {
        throw new Error(
          "Phiên đăng nhập đã hết hạn hoặc thiếu access token. Vui lòng đăng nhập lại.",
        );
      }

      if (response.status === 403) {
        throw new Error(
          "Bạn không có quyền truy cập màn hình duyệt KYC. Chỉ Regulatory/Admin được phép.",
        );
      }

      if (!response.ok)
        throw new Error(
          responseData?.message || "Không thể tải danh sách hồ sơ KYC.",
        );

      // Ghi chú logic phức tạp: chuẩn hóa dữ liệu fallback để UI luôn hiển thị đủ thông tin chính.

      const normalizedSubmissionList = (responseData?.submissions || []).map(
        (submissionItem: any) => ({
          ...submissionItem,
  ...
  const submitKycReview = useCallback(
    async (action: "approve" | "reject") => {
      ...
      try {
        ...
        const response = await fetch(
          `${backendBaseUrl}/auth/organization/kyc-submissions/${selectedSubmission.submissionId}/review`,
          {
            method: "PATCH",
        ...
        if (response.status === 403) {
          throw new Error(
            "Bạn không có quyền duyệt hồ sơ KYC. Chỉ Regulatory/Admin được phép.",
          );
        }

        if (!response.ok)
          throw new Error(
            responseData?.message || "Cập nhật trạng thái hồ sơ thất bại.",
          );
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg border border-slate-200 p-3">
                <p className="mb-3 text-xs font-semibold text-slate-800">
                  Danh sách tài liệu KYC (CID/IPFS)
                </p>
                ...
                <div className="space-y-3">
                  {selectedSubmission.files.map((fileItem, fileIndex) => (
                    ...
                    >
                      <p className="text-xs font-semibold text-slate-900">
                        Tài liệu #{fileIndex + 1}: {fileItem.fileName}
                      </p>
